...
window_duration = 0.064;
overlap_duration = 0.032;

window_samples = round(window_duration * fs_new);
overlap_samples = round(overlap_duration * fs_new);
...
avg_peaks_per_sec = total_peaks / total_duration;

fprintf('Ukupno: %d\n', total_peaks);
fprintf('Prosje?no: %.2f\n', avg_peaks_per_sec);
...
[freq_ind, time_ind] = find(P_thresholded);

num_peaks = length(time_ind);
hash_table = [];
...
for i = 1:num_peaks
    ...
    min_time_idx = at_idx + round(delta_t_min_sec / (T(2)-T(1)));
    max_time_idx = at_idx + round(delta_t_max_sec / (T(2)-T(1)));
    
    min_freq_idx = af_idx - round(delta_f_hz / (F(2)-F(1)));
    max_freq_idx = af_idx + round(delta_f_hz / (F(2)-F(1)));
